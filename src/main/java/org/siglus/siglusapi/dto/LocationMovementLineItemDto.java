...
@NamedNativeQueries({
    @NamedNativeQuery(name = "LocationMovement.getStockMovementWithLocation",
        query = "select\n"
            ...
            + "      when pili.quantity-pili.previousstockonhandwhensubmitted>0 then '[Ajustes Positivos] '\n"
            ...
            + "    when scli.sourceid is null\n"
            + "    and scli.destinationid is null\n"
            + "    and scli.reasonid is null and pili.previousstockonhandwhensubmitted is not null \n"
            + "    then pili.quantity-pili.previousstockonhandwhensubmitted\n"
            + "    else scli.quantity\n"
            + "  end as quantity ,\n"
            + "  scli.occurreddate ,\n"
            + "  scli.processeddate ,\n"
            ...
            + "left join stockmanagement.physical_inventory_line_items pili on\n"
            + "  pili.physicalinventoryid = phi.id\n"
            + "  and pili.orderableid = sc.orderableid\n"
            + "  and (case\n"
            + "    when pili.lotid is not null then pili.lotid\n"
            + "    else '0ad1d4ce-38b3-11ed-a261-0242ac120002'\n"
            + "  end) = (case\n"
            + "    when sc.lotid is not null then sc.lotid\n"
            + "    else '0ad1d4ce-38b3-11ed-a261-0242ac120002'\n"
            + "  end)\n"
