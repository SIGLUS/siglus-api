...
public interface SiglusStockCardRepository extends JpaRepository<StockCard, UUID>, JpaSpecificationExecutor<StockCard> {
  ...
  List<StockCard> findByFacilityIdAndOrderableIdAndLotId(
      @Param("facilityId") UUID facilityId,
      @Param("orderableId") UUID orderableId,
      @Param("lotId") UUID lotId);
  ...
  @Query(value = "select * from stockmanagement.stock_cards where facilityid=:facilityId "
      + "and concat(cast(programid as varchar), concat(cast(orderableid as varchar),cast(lotid as varchar))) in :pairs",
      nativeQuery = true)
  List<StockCard> findByFacilityIdAndProgramOrderableLotIdPairs(
      @Param("facilityId") UUID facilityId,
      @Param("pairs") Set<String> programOrderableLotIdPairs);
  ...
  @Query(value = "select cast(lotid as varchar) from stockmanagement.stock_cards "
      + "where facilityid = :facilityId and orderableid in :orderableIds ",
      nativeQuery = true)
  List<String> findLotIdByFacilityAndOrderables(@Param("facilityId") UUID facilityId,
                                                      @Param("orderableIds") Collection<UUID> orderableIds);
}
