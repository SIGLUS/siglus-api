...
public class LocationDraftRepository extends BaseNativeRepository {
  ...
  private static final String SQL_13_1 = "UPDATE fulfillment.orders SET status = 'PARTIALLY_FULFILLED' WHERE id IN (SELECT orderid FROM fulfillment.shipment_drafts WHERE orderid IN (SELECT id FROM fulfillment.orders WHERE supplyingfacilityid = ?)) and ordercode ~ '^OF[A-Za-z0-9.]*/(0[2-9]|[1-9][0-9])$'";
