...
drop view if exists dashboard.vw_fulfillments;
create view dashboard.vw_fulfillments
            (
             rliid,
             approvedquantity,
             district,
...
-- Regular
    (select t.rliid,
            ...
            t.originalperiod,
            t.productname,
            (case when t.productcode in (select code from temp_malaria_product_code) is true then 'Malaria'
                  when t.productcode in (select code from temp_mmc_product_code) is true then 'Material Medico Cirúrgico'
...
(select distinct on (t.ficilitycode, t.periodid, t.productcode) t.rliid,
    ...
    t.originalperiod,
    t.productname,
    'Material Medico Cirúrgico' as program,
    'MMC'                       as programcode,
    t.province,
    t.provincefacilitycode,
    t.reporttype,
 ...
 from temp_all_line_items t
 where t.programcode = 'VC'
   and t.reporttype = 'Emergency' and t.productcode in (select code from temp_mmc_product_code))
);
