...
CREATE VIEW dashboard.vw_mmia_regimens AS  SELECT DISTINCT ON (rq.id, rgli.regimenid) gz_prov.name AS province,
   ...
   FROM requisition.requisitions rq
     ...
     LEFT JOIN (
        ...
        CASE
            WHEN totalpatients = 0 OR totalpatients IS NULL OR patientsintreatment IS NULL THEN 0::numeric
            ELSE round(patientsintreatment / totalpatients, 2)
        END AS correctionfactor
        FROM siglusintegration.patient_line_items pli
        left join
        (
            select requisitionid, value::numeric as totalpatients from siglusintegration.patient_line_items
            where groupname::text = 'newSection5' and columnname::text = 'total'
        ) as tps on pli.requisitionid = tps.requisitionid
        left join
        (
            select requisitionid, value::numeric as patientsintreatment from siglusintegration.patient_line_items
            where groupname::text = 'newSection6' and columnname::text = 'total'
