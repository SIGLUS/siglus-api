...
drop
materialized view if exists dashboard.vw_historical_data;
create
materialized view dashboard.vw_historical_data as
...
from requisition.requisition_line_items rli
    ...
    p.id = r.programid
         left join (select *
                    ...
                    where o.versionnumber = o.latestversion) o
                   on
    ...
    pp.id = r.processingperiodid
     ...
     siglusintegration.hf_cmms hc on
         ...
         left join (select e.facilityid,
                           e.periodid,
                           e.orderableid,
                           sum(e.expiredquantity) as expiredquantity
                    from (select r.facilityid,
                                 rli.orderableid,
                                 pp.id            as periodid,
                                 sc.lotid,
    ...
    pp.processingscheduleid = ps.id
         left join (select r.processingperiodid,
                           r.facilityid,
                           rli.orderableid,
                    ...
                    from requisition.requisition_line_items rli
                             join requisition.requisitions r on
                        r.id = rli.requisitionid
                    group by r.processingperiodid,
                             r.facilityid,
                             rli.orderableid) appandreq
                   on appandreq.processingperiodid = pp.id and appandreq.facilityid = r.facilityid and
                      appandreq.orderableid = rli.orderableid
where ps.code in ('M1', 'Q1')
