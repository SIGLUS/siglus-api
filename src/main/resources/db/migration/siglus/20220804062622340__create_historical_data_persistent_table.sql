...
select
    ...
    case
        when hc.cmm is null
            or hc.cmm <= cast(0 as double precision) then cast(0 as numeric)
        else round(cast(closesoh.closestock as numeric) / round(cast(hc.cmm as numeric), 2), 1)
        end
                        as mos,
    case
    ...
    (
        ...
        sum(sc.adjustment) OVER (PARTITION BY main.periodid, main.facilityid, sc.orderableid) AS totaladjustments
                        FROM ( SELECT pp_1.id AS periodid,
                            ...
                            WHERE ps.code in ('M1','Q1') AND sc_1.lotid IS NOT NULL AND pp_1.enddate < now()
                            ...
                            END AS consumption,
                            CASE
                            WHEN scli.sourceid IS NULL AND scli.destinationid IS NULL AND scli.reasonid IS NOT NULL AND sclir.reasontype = cast('DEBIT' as text) THEN - scli.quantity
                            WHEN scli.sourceid IS NULL AND scli.destinationid IS NULL AND scli.reasonid IS NOT NULL AND sclir.reasontype <> cast('DEBIT' as text) THEN scli.quantity
                            END AS adjustment
