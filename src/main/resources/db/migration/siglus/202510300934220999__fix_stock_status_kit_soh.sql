...
AS WITH temp_malaria_product_code AS (
   ...
   FROM (SELECT
             ...
             SUM(
                     CASE
                         WHEN l.expirationdate IS NULL OR l.expirationdate >= CURRENT_DATE
                             THEN csoh.stockonhand
                         ELSE 0
                         END
             ) AS totalsoh,
             MAX(csoh.processeddate) AS processeddate,
             MIN(CASE
                     WHEN csoh.stockonhand > 0
                         AND l.expirationdate >= CURRENT_DATE
                         THEN l.expirationdate
                 END) AS expirationdate
         FROM stockmanagement.stock_cards sc_1
                  LEFT JOIN dashboard.vw_calculated_stocks_on_hand csoh
                            ON csoh.stockcardid = sc_1.id
                  LEFT JOIN referencedata.lots l
                            ON l.id = sc_1.lotid
         WHERE l.expirationdate IS NULL OR l.expirationdate >= CURRENT_DATE
         GROUP BY
             sc_1.facilityid,
             sc_1.programid,
             sc_1.orderableid) tsd
