...
from
	...
	from
		...
		select
			...
			sc.lotid
		order by
			sc.occurreddate desc) as closestockindex
		from
			(
			select
				pp.id as periodid,
			...
			cross join (
				...
				from
					stockmanagement.stock_cards) scf
			where
				pp.enddate<now()
	) main
		left join (
			...
			left join stockmanagement.calculated_stocks_on_hand csoh on
				csoh.stockcardid = sc.id ) sc on
			sc.facilityid = main.facilityid
			and sc.occurreddate <= main.enddate
		where
			sc.lotid is not null) a
	where
		closestockindex = 1 ) closesoh
left join (
	select
		distinct on
		(a.periodid,
	...
	from
		...
		select
			...
			sc.lotid
		order by
			sc.occurreddate desc) as initialstockindex
		from
			(
			select
				pp.id as periodid,
