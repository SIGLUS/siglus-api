...
SELECT DISTINCT ON (soh.facilityid, soh.orderableid)
    ...
    WHEN prnm.programname::text = 'TARV'::text AND (h.cmm * 3::double precision) < soh.totalsoh::double precision THEN
        'Stock acumulado'::text
    WHEN prnm.programname::text <> 'TARV'::text AND (h.cmm * 2::double precision) < soh.totalsoh::double precision THEN
        'Stock acumulado'::text
    ELSE
        'Stock regular'::text
    END AS stockstatus,
