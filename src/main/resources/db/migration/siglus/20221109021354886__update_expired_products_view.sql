...
drop view if exists dashboard.vw_expired_products;
CREATE VIEW dashboard.vw_expired_products AS
...
SELECT pgz.name                                                         AS provincename,
       ...
       (CASE
            WHEN o.code in
                 (SELECT code
                  FROM temp_malaria_product_code) IS TRUE THEN 'Malaria'
            WHEN o.code in
                 (SELECT code
                  FROM temp_mmc_product_code) IS TRUE THEN 'Material Medico Cirúrgico'
            ELSE p.name
        END)                                                            AS programname
