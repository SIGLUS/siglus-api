...
CREATE OR REPLACE VIEW dashboard.vw_expiring_products
AS WITH temp_malaria_product_code AS (
   ...
   SELECT pgz.name AS provincename,
          ...
          CASE
              WHEN l.expirationdate >= CURRENT_DATE AND l.expirationdate <= (CURRENT_DATE + '30 days'::interval) THEN 1
              ELSE 0
              END AS expireinonemonth,
          CASE
   ...
   WHERE l.expirationdate >= CURRENT_DATE AND l.expirationdate <= (CURRENT_DATE + '180 days'::interval) AND scoh.stockonhand > 0;
