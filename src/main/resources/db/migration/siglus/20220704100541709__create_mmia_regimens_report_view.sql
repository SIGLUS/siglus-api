...
select gz_prov.name            as province,
       ...
       case
           when rq.emergency = true then 'Emergency'
           else 'Periodical'
           end                 as requisitiontype,
       rgli.value              as patients,
       rg.name                 as regimenname,
       re.requisitionnumberprefix::text || lpad(re.requisitionnumber::text, 7, '0') as requisitionnumber, p."name" as program,
...
from requisition.requisitions rq
         left join (select *
                    ...
                    where columnname = 'patients') rgli on
    rgli.requisitionid = rq.id
         left join (select rli.requisitionid,
				...
				else round(pli.value::numeric / rli.value::numeric, 2)
                    ...
                    from
                        ...
                        *
                        ...
                        and columnname = 'total') pli
                    on
                        rli.requisitionid = pli.requisitionid) ttp on
         ...
         left join (select *,
                           concat(to_char(startdate::timestamp with time zone, 'DD'::text), ' ',
                                  case to_char(startdate::timestamp with time zone, 'MM'::text)
                                      ...
                                      end, ' ', to_char(startdate::timestamp with time zone, 'YYYY'::text), ' - ',
                                  to_char(enddate::timestamp with time zone, 'DD'::text), ' ',
                                  ...
                                  to_char(enddate::timestamp with time zone, 'YYYY'::text)) as requisitionperiod
                    from referencedata.processing_periods) pp on
    ...
    p.id = rq.programid
where rgli.regimenid is not null
  and p.code = 'T';