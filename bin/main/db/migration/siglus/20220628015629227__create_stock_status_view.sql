...
as mos,
	case
		when soh.totalsoh = 0 then 'Estoque'::text
		when h.cmm = '-1'::integer::double precision
		or h.cmm is null then 'Estoque Regular'::text
		when soh.totalsoh::double precision < h.cmm then 'Baixo estoque'::text
		when prnm.programcode::text = 'TARV'::text
		and (h.cmm * 3::double precision) < soh.totalsoh::double precision then 'estoque excessivo'::text
		when prnm.programcode::text <> 'TARV'::text
		and (h.cmm * 2::double precision) < soh.totalsoh::double precision then 'estoque excessivo'::text
		else 'Estoque Regular'::text
end
as stockstatus,
